<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <form class="form-inline" action="{{route('courses.search')}}">
            <button class="btn btn-outline-success my-2 my-sm-0 search-btn border-right-0 " type="submit"><i class="fas fa-search"></i></button>
        <input class="form-control mr-sm-2 search-input border-left-0 border-top-0 border-bottom-0" type="search" placeholder="Search" aria-label="Search" name="search" value="{{request()->search ?? ''}}">
        </form >
    </div>
</nav>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item {{ (Illuminate\Support\Facades\Route::currentRouteName())=='more-course'? 'active' :'' }} ">
                    <a class="nav-link" href="{{ url('/more-course') }}">More Courses<span class="sr-only">(current)</span></a>
                </li>
                <li class="nav-item {{ (Illuminate\Support\Facades\Route::currentRouteName())=='carte.index'? 'active' :'' }} ">
                    <a class="nav-link" href="{{ url('/shoppingcart') }}">({{session()->has('cart')?session()->get('cart')->totalQty :  '0'}}) Shopping Carte<span class="sr-only">(current)</span></a>
                </li>
                @if (auth()->user())
                @if (auth()->user()->account_type=="professor")
                 <li class="nav-item {{ (Illuminate\Support\Facades\Route::currentRouteName())=='course'? 'active' :'' }}">
                    <a class="nav-link" href="{{ route('course') }}">My Courses</a>
                 </li>

                @else
                  <li class="nav-item {{ (Illuminate\Support\Facades\Route::currentRouteName())=='home'? 'active' :'' }}">
                    <a class="nav-link" href="{{ route('home') }}">My Courses</a>
                 </li>
                @endif
                @endif
            ...
            <ul class="navbar-nav ml-auto">
                ...
                @else
                  <li class="nav-item {{ (Illuminate\Support\Facades\Route::currentRouteName())=='order.index'? 'active' :'' }} ">
